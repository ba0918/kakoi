...
- Linux on x86_64, including WSL2 (no aarch64 yet)
...
- For the `filtered` network mode only: `pasta` (the `passt` package; tested with Debian
  ...
  A distribution's `pasta` can be too old; [Installing pasta](docs/pasta.md) shows how to check
  yours and build a newer one.
...
- [Security model](docs/security.md): trust boundary, known gaps, and what is not supported yet.
...
Among the things `kakoi` does not do yet: no cgroup limits, no multicast or
...
[Not supported yet](docs/security.md#not-supported-yet).
