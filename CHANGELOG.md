...
- Added `skills/process-wrap-setup/SKILL.md`, an Agent Skill that proposes the machine-specific
  ...
  writing. It asks first whether the profile, and the shim it would place, are kept elsewhere,
  and where they are it proposes the lines or the file instead of writing; it names the options
  the template cannot copy and the way around each, checks on every route the command is
  started from that the shim is found first, and never lists `secrets/`.
