...
IPv6リンクローカル宛先を個別に許可するときのネットワーク指定を定義する草案。接続口の名前変更、隔離環境からの経路、広いCIDRで含む場合は未決。初版では提供せず後続へ分けた（未修正のpastaがリンクローカル宛て通信を運ばないため）。

- deferred: docs/decision/brainstorm/2026-09-15-kakoi-net.md#A167
