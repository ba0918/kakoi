...
今回追加するネットワーク設定を使う場合は、空の許可・公開配列や補助設定だけでも "mode" の明示を必須とする。別の段から引き継いだ明示モードも認める。新設定を一切使わない既存設定の扱いは変えない。
...
Scenario: 下の段のモード指定も認める
  Given 下の段にmode="filtered"がある
  And 上の段にはnetwork.publishの空配列だけを指定している
  When 起動前に設定を検査する
