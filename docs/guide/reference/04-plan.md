...
## 全量（full）の中身
<!-- @kotowari[REQ-297:a9044fcc, EX-533:9a60d7bf] -->
...
## 要約と全量の両方に出るもの
<!-- @kotowari[REQ-297:a9044fcc] -->

次の 4 つは、要約と全量のどちらでも同じように表示する。
...
## JSON のキー
<!-- @kotowari[REQ-299:d9edf900, EX-535:239c4e1a] -->

トップレベルのキーは次の 16 個である。
...
`kakoi` は起動の前（`--print-plan` でも同じ）に検査を段階の順に行い、最初に当たった診断で終わる。
段階の全体は[入れ子、並列、出力、診断と終了コード](10-process.md)が定める。
ここでは、そのうちマウントを解決する段階の中の順序を示す。
...
各検査の中身は[マウントの解決](07-mounts.md)と[ポリシーを保護する配置](06-policy-placement.md)、秘密と `GIT_CONFIG_COUNT` は[環境変数](08-environment.md)を参照する。
