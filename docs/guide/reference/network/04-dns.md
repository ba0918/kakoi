...
## 応答の検査
<!-- @kotowari[REQ-019:24f6a84e, REQ-023:02998f2d, REQ-020:ffa58c43] -->
...
### 許可できない IP が混ざった応答
<!-- @kotowari[REQ-020:ffa58c43, EX-031:81bc058a, EX-032:be2a5b9d, EX-033:20b35958, EX-806:fcf4fc9b, EX-807:ef6e08d2] -->
...
ここでの署名付きは、次のどちらかに当たる応答を指す。

- メッセージ署名（TSIG）が付いた応答
- DNSSEC の署名（RRSIG）を含む応答

TSIG が無く RRSIG だけを含む応答でも、混在していれば加工せずに返す。
...
### 許可は期限より後に切れない
<!-- @kotowari[REQ-397:6adfab5c, REQ-420:6ac58bed, EX-733:e6a358bf, EX-804:a3275526, EX-805:7dcfc537] -->

DNS 由来の許可は、応答の受信と TTL から決まる期限より後まで、カーネルに残さない。
許可はカーネルに残り時間で登録するため、登録にかかる時間を見込んだ分だけ、期限より早く失効することはある。

登録が見込んだ時間の内に終わらなければ、見込む時間を倍にして登録し直す。
見込む時間の上限は、登録する許可のうち最も早い期限までの残り時間の半分である。
例えば最も早い期限まで残り 1 秒なら、倍にした結果が 0.5 秒を超えても、0.5 秒より長くは見込まない。
登録し直す場合も、カーネルの許可は期限までに失効する。
...
### アプリへ返す TTL
<!-- @kotowari[REQ-398:4e76855f, EX-734:b89ae834, EX-735:10313912, EX-802:3d20d086, EX-803:4ba34ad5] -->
...
メッセージ署名（TSIG）付きの応答は書き換えず、元の TTL のまま返す。
TTL を揃えないのは TSIG 付きの応答だけで、DNSSEC の署名（RRSIG）だけを持つ応答の TTL は揃える。
混在した応答を加工するかどうかの判断では RRSIG も署名付きに数えるので（[許可できない IP が混ざった応答](#許可できない-ip-が混ざった応答)）、2 つの規則で署名の範囲が異なる。

| 上流の応答（許可された 2 つの IP が TTL 30 秒と 300 秒） | アプリが受け取る TTL |
|---|---|
| TSIG 付き | 30 秒と 300 秒のまま |
| TSIG が無く RRSIG を含む | どちらも 30 秒以下 |
...
### 上流との通信方式
<!-- @kotowari[REQ-131:2b313183, REQ-132:ad9b7c8f, EX-297:7a78c7db] -->
