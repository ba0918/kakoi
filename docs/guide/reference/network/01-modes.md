...
- 必須のキーが欠けている（例: 公開の指定に `host-port` が無い）
- ポート番号が不正である（例: 通信許可の `ports` に `"65536"` がある）
- 公開の指定どうしが競合する（例: 同じ内側の TCP 8000 番を、ホスト側の 18000 番と 19000 番へ公開する 2 つの指定がある）
...
## モードに関わるエラー
<!-- @kotowari[REQ-084:a384542a, REQ-395:e8bab658, EX-171:00b16a66] -->
