...
明示した上流DNSの1候補を待つ時間は既定2秒、名前解決全体の上限は既定10秒とし、どちらも設定で変更できる。別候補への切替、CNAME参照、ホストDNS設定変更によるやり直しで全体の期限を延長しない。ホストDNS利用時も全体上限を適用する。ホストの名前解決設定の `nameserver` をkakoiが順に試す場合は各候補に1候補の待ち時間を適用し、systemd-resolvedの中継窓口だけを使う場合は中継側が候補を選ぶため、1候補の待ち時間を適用せず全体上限まで待つ。
